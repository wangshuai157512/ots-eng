                        ...
                        <el-form-item label="">
                            ...
                            <div class="optionsBox mr30">
                                ...
                                <el-checkbox-group class="p20 pt0" v-model="siteSelectList" @change="handleCheckedSite">
                                    ...
                                    <el-checkbox v-for="item in siteSelectData" :label="item" :key="item.id">{{`${item.name}(${item.classList.length})`}}</el-checkbox>
                                </el-checkbox-group>
                            </div>
                            <div class="optionsBox">
                                ...
                                <el-checkbox-group class="p20 pt0" v-model="classSelectList" @change="handleCheckedClass">
                                    <el-checkbox v-for="item in classSelectData" :label="item" :key="item.id">{{`${item.name}(${item.seatNumber})`}}</el-checkbox>
                                </el-checkbox-group>
                            </div>
                        </el-form-item>
                    </div>
    ...
    methods: {
        ...
        async initPage () {
            if (this.$route.query.id) {
                ...
                let classIdList = flatten(placeList.map(item => item.classIdList))
                // console.log(classIdList)
                let temp = flatten(this.siteSelectList.map(item => item.classList))
                
                this.classSelectList =  temp.filter(item => classIdList.filter(bb => bb == item.id).length > 0)
        ...
        paperBatchOper (val) {
            let idList=[...this.paperSelectList]
            val.forEach(item=>{
                let isHave = idList.filter(selectItem => selectItem.id == item.id).length
                if (!isHave) {
                    idList.push(item)
                }
            })
        ...
        async getPaper () {
            ...
            this.paperPagination.total = data.total
        },
        dealCoursePaper () {
            ...
            this.cpList.forEach(item => {
                let isHave = temp.some(tempItem => tempItem.id === item.id)
                if (!isHave) {
                    temp.push(item)
                }
            })
        ...
        deleteCoursePaper (row) {
            this.cpList = this.cpList.filter(item => item.paperId != row.paperId)
            this.paperSelectList = this.paperSelectList.filter(item => item.paperId != row.paperId)
        },
        addOrEdit () {
